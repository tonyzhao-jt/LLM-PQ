...
CONT_NAME="shaq" 
docker build -t juntao/${CONT_NAME}:0.1 .
...
docker run --net=host --gpus all --name $CONT_NAME -v $DATA_PATH:/data -v $ROOT_PATH:/workspace/${CONT_NAME} -it juntao/${CONT_NAME}:0.1 
